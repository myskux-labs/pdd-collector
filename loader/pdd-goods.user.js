...
// @require 让业务代码运行在与本脚本相同的沙箱作用域中，GM_download / GM_setClipboard / GM_xmlhttpRequest
// 等 API 才能正常工作——不要改回用 GM_xmlhttpRequest 拉取后 new Function()/eval 执行，那样业务代码
// 拿不到 GM API 所在的闭包，会导致 GM_download 看起来调用成功但实际不会真正触发下载。
//
// @connect * 是必须的：下载改成先用 GM_xmlhttpRequest 抓取轮播主图/详情页图片的二进制数据、
// 用 fflate 打包成 zip 后再整体下载。拼多多的图片 CDN 域名很多且会变化（t00img/imagev2.pddpic.com
// 等），无法逐个枚举，而 GM_xmlhttpRequest（不同于 GM_download）会真正校验 @connect 域名白名单，
// 域名不在白名单时请求会被静默拦截。
//
